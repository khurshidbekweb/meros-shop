...
function View() {  
  let delivery = true;
  const [mainImg, setMainImg] = useState(data[0].img);
            ...
            <div className="img-wrap w-[50%] flex ">
              <div className="child-img w-[20%] text-center">
                {data.length && data.map((item, i)=>{
                  return (
                    <img key={item.id} onClick={()=> setMainImg(item.img)} width={90} height={110} className={ i == item.id ? `mb-2 border p-1 cursor-pointer active-child-img` : `mb-2 border p-1 cursor-pointer`} src={item.img} alt="img" />
                  )
                })}
              </div>
              <div className="main-img-view  w-[500px]">
                  <img  className="mb-2 border px-10 py-5"  src={mainImg} alt="img" />
              </div>
            </div>
            <div className="view-information-praduct px-5">
              <h2 className="text-[24px] font-bold mb-3">Триллионер слушает</h2>
              <Degrre/>
              <h3 className="text-[24px] font-bold mt-5">Коротко о товаре</h3>
              <p className="text-[18px] text-[#8D909B]">Автор  _ _ _ _ _ _ _ _ _ _ _ <strong className="text-black">Аляутдинов Шамиль Рифатович</strong></p>
              <p className="text-[18px] text-[#8D909B]">Языки  _ _ _ _ _ _ _ _ _ _ _ <strong className="text-black">Русский</strong></p>
              <p className="text-[18px] text-[#8D909B]">Надпись  _ _ _ _ _ _ _ _ _ _ <strong className="text-black">Кириллица</strong></p>
              <p className="text-[18px] text-[#8D909B]">Издательство  _ _ _ _ _ _ _  <strong className="text-black">Диля</strong></p>
              ...
              <h2 className="text-[35px] font-bold text-[#32386B] mt-5">{count*50000} сум</h2>   
              <div className="btn-wrap-view mt-4 flex gap-5">
                <button className="hover:bg-[#32386B] hover:text-white transition-all p-3 px-12 text-[20px] font-medium border rounded-[5px]">Купить</button>
                <button className="hover:bg-[#32386B] hover:text-white transition-all p-3 px-12 text-[20px] font-medium border rounded-[5px]">В корзину</button>
              </div>
              <div className="fovorite flex gap-4 mt-7">
                <img className="cursor-pointer" src={fovorite} alt="fovorite" />
